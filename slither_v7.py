...
Snake = Snake(screen,Bg,DIR_INPUT+"\\snakehead_blue.png",DIR_INPUT+"\\snakebody_blue.png",speed, initial_position)
...
while running:
    ...
    Snake.set_screen_center(position)
    ...
    # Crash Judge
    # Lose
    tmp_list_length = [math.sqrt((tmp_position[0] - position[0])**2 + (tmp_position[1] - position[1])**2) for tmp_position in Snake.list_hist]
    min_length = min(tmp_list_length)
    print("Snake Head Min Length",min_length)
    if min_length < 10:
        print("You lose!!")
        sys.exit()
    # Kill Snake
    tmp_list_length = [math.sqrt((Snake.list_hist[0][0] - tmp_position[0])**2 + (Snake.list_hist[0][1] - tmp_position[1])**2) for tmp_position in Player.list_hist]
    min_length = min(tmp_list_length)
